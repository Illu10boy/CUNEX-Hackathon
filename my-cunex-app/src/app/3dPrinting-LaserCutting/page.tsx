    ...
    <div className="min-h-screen bg-pink-50">
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="flex justify-between mb-8">
          ...
          <div className="flex-1 text-center px-4">
            <div className="w-10 h-10 rounded-full bg-pink-500 text-white flex items-center justify-center mx-auto mb-2 font-bold">2</div>
            <div className="text-gray-800 font-semibold">Enter Details</div>
          </div>
          <div className="flex-1 text-center px-4">
            <div className="w-10 h-10 rounded-full bg-pink-200 text-pink-500 flex items-center justify-center mx-auto mb-2 font-bold">3</div>
        ...
        <div className="bg-white rounded-2xl p-8 shadow-lg mb-8">
          ...
          <form>
            <div className="mb-6">
              <label className="block text-pink-500 font-semibold mb-2" htmlFor="file-upload">
                Upload 3D Model/Laser printing file
              </label>
              <div 
                className={`border-2 ${selectedFile ? 'border-pink-500 bg-pink-50' : 'border-pink-200 border-dashed'} rounded-lg p-8 text-center cursor-pointer transition-all hover:border-pink-500 hover:bg-pink-50`}
                onClick={() => fileInputRef.current?.click()}
              >
                <div className="text-4xl mb-4">{selectedFile ? '✅' : '📤'}</div>
                {selectedFile ? (
                  <p className="text-gray-800">File selected: {selectedFile.name}</p>
                ) : (
                  <>
                    <p className="mb-2 text-gray-800">Drag & drop your file here or click to browse</p>
                    <p className="text-sm text-gray-600">Supported formats: STL, DXF</p>
                  </>
                )}
                <input 
                  ref={fileInputRef}
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {materials.map((material) => (
                  <div 
                    key={material.id}
                    className={`border-2 ${selectedMaterial === material.id ? 'border-pink-500 bg-pink-50' : 'border-pink-200'} rounded-lg p-4 text-center cursor-pointer transition-all hover:border-pink-500 hover:bg-pink-50`}
            ...
            <div className="mb-6">
              <label className="block text-pink-500 font-semibold mb-2" htmlFor="size">
                Size & Specifications
              </label>
              <textarea 
                id="size" 
            ...
            <div className="mb-6">
              <label className="block text-pink-500 font-semibold mb-2" htmlFor="additional">
                Additional Requests
              </label>
              <textarea 
                id="additional" 
