...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const [rows] = (await con.execute("SELECT userId FROM users WHERE studentId = '6538114021'")) as any;

        await con.end(); // Close connection

        if (!Array.isArray(rows) || rows.length === 0) {
            return NextResponse.json({ success: false, message: "No users found with studentId 6538114021" }, { status: 404 });
        }
        ...
        const payload = {
            data: rows.map((row: any) => ({
                ...
                text_EN: "There is a request for fabrication sent by a customer, tap to see detail.",
                callbackUrl: "https://cunexhackathon.vercel.app/service/fabrication/serviceProvider"
            }))
        };
        ...
        console.log(rows.length, "users found with studentId 6538114021");
